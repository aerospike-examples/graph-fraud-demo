...
export default function Performance() {
  ...
  const runPerformanceTest = async (
    ...
    try {
      ...
      const response = await fetch(
        `/api/performance/test/${method}?transaction_count=${count}`,
        {
          method: "POST",
        }
      );
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to run ${method.toUpperCase()} performance test`
        );
      }
      ...
      setError(
        err instanceof Error ? err.message : "Failed to run performance test"
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const renderMethodCard = (
    ...
    const methodInfo = {
      rt1: {
        name: "RT1",
        description: "Flagged Account Detection",
        icon: Shield,
        color: "bg-blue-500",
      },
      rt2: {
        name: "RT2",
        description: "Flagged Account Multi-hop Detection",
        icon: Database,
        color: "bg-green-500",
      },
      rt3: {
        name: "RT3",
        description: "Flagged Device Detection",
        icon: Target,
        color: "bg-purple-500",
      },
    };
  ...
  const renderTimelineChart = () => {
      ...
      return (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Real-time execution times over the last {timeWindow} minutes
            </CardDescription>
          </CardHeader>
          <CardContent>
    ...
    const createLinePath = (data: TimelineData[], width: number) => {
      ...
      const points = recentData.map((point, index) => {
        ...
        const y =
          chartHeight -
          ((point.execution_time - chartMinTime) / chartRange) * chartHeight;
        return `${x},${y}`;
      });
                        ...
                        <g key={method}>
                          ...
                          {data
                            ...
                            .map((point, index) => {
                              ...
                              const x =
                                (actualIndex / (recentData.length - 1 || 1)) *
                                800;
                              const y =
                                chartHeight -
                                ((point.execution_time - chartMinTime) /
                                  chartRange) *
                                  chartHeight;
          ...
          <div className="px-6 py-3 border-t bg-gray-50/30">
            <div className="grid grid-cols-3 gap-4">
              {methods.map((method) => {
                ...
                const avgTime =
                  recentData.reduce((sum, d) => sum + d.execution_time, 0) /
                  recentData.length;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Real-time monitoring of Gremlin query latencies for RT1, RT2, and
            RT3 fraud detection methods
          </p>
        </div>
        <div className="flex items-center space-x-2">
          <Button
