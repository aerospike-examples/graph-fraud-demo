...
const Search = ({
  ...
  const handleChange = (e: ChangeEvent) => {
    ...
    setFilteredAccts(
      accounts.filter((item) => item.account_id.startsWith(val.toUpperCase()))
    );
    setSearch(val);
    setShow(true);
  };
  ...
  // Update filtered accounts when accounts prop changes, but only if it's actually different
  useEffect(() => {
          ...
          {filteredAccts.length > 0 ? (
            filteredAccts.slice(0, 10).map(({ account_id, account_type }) => (
              <div
                ...
                onClick={() =>
                  handleSelect({ id: account_id, type: account_type })
                }
              >
