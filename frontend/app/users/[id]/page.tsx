...
export default async function UserDetailPage({
  ...
  const { user, risk_level, ...userDetails }: UserSummary =
    await response.json();
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent className="grid grid-cols-2 gap-4">
            <Label size="lg" title="Full Name" text={user.name} />
            <Label size="lg" title="Age" text={`${user.age} years`} />
            <Label size="sm" title="Email" text={user.email} icon="mail" />
            <Label size="sm" title="Phone" text={user.phone} icon="phone" />
            <Label
              size="lg"
              title="Location"
              text={user.location}
